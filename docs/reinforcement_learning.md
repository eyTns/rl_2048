...
**TD (Temporal Difference) 학습**이 핵심이다.

#### Step 1: 경험 수집
```
(s, a=하, r=256, s', done=False)
```
- s: 현재 상태 (주어진 보드)
- a: 선택한 행동 (아래)
...
#### Step 2: 타겟 Q값 계산 (벨만 방정식)
```
Target = r + γ × max Q(s', a')
       = 256 + 0.99 × max{Q(s', 상), Q(s', 하), Q(s', 좌), Q(s', 우)}
       = 256 + 0.99 × 2500  (예시: 다음 상태에서 최대 Q가 2500)
       = 2731
```

#### Step 3: 손실(Loss) 계산
```
현재 예측: Q(s, 하) = 2200  (모델의 현재 예측, 아직 부정확)
Target:    2731
...
#### Step 4: 역전파로 가중치 업데이트
```
Q(s, 하) ← 2200 → 약 2253 방향으로 조정 (learning rate에 따라)
```
...
```
[경험 수집]
    게임 플레이 → (s, a, r, s') 저장 → Replay Buffer에 축적
         ↓
[학습]
    Buffer에서 랜덤 샘플 → 벨만 방정식으로 Target 계산
         ↓
    Loss = (Target - 현재 예측)² → 역전파 → 가중치 업데이트
         ↓
[반복]
