# PR 2: 게임 플레이 UI 리뷰

## 리뷰 내용

- [ ] 구현된 UI는 컴퓨터로 플레이하는 것인가? 모바일로 어떻게든 플레이할 수 있는 방법이 있는가?
- [ ] game_ui.py의 모든 함수 설명듣기

## 답변

### 컴퓨터 vs 모바일 플레이

현재 game_ui.py는 **컴퓨터 전용** (pygame, 키보드 입력).

모바일 지원을 위해 **웹 기반 UI (web_ui.py)** 를 별도 구현.

| UI | 파일 | 용도 |
|----|------|------|
| 컴퓨터 UI | game_ui.py | pygame 기반, 키보드 조작 |
| 모바일 UI (서버) | web_ui.py | FastAPI 기반, ML 연동용 |
| 모바일 UI (정적) | static_ui.html | 서버 불필요, GitHub Pages 배포 가능 |

모듈 구조:
- game2048.py: 게임 로직 (Python, RL 학습용)
- game_ui.py: 컴퓨터 UI
- web_ui.py: 서버 기반 웹 UI (ML 연동)
- static_ui.html: 정적 웹 UI (독립 플레이)
- (추후) ml_ui.py: 머신러닝 시각화

## 코드 리뷰

### game_ui.py

| 라인 | 이슈 | 심각도 | 상태 |
|------|------|--------|------|
| 37 | `GRID_COLOR` 선언됐지만 미사용 | Low | 해결 |

### web_ui.py

| 라인 | 이슈 | 심각도 | 상태 |
|------|------|--------|------|
| 6 | 전역 `game` 인스턴스 → 멀티 유저 미지원 | Medium | 해결 (ML 연동용 단일 유저로 충분, 외부 공개 안 함) |
| 101 | `action` 범위 검증 없음 (0-3) | Medium | 해결 |

### static_ui.html

| 라인 | 이슈 | 심각도 | 상태 |
|------|------|--------|------|
| 79-90 | `rotateBoard` 시계 방향 (Python 반시계) | Medium | 해결 (rotation 매핑 조정) |
| 65-77 | 게임 로직 Python/JS 중복 | Info | 인지 (구조적 한계) |

