...
- [ ] DQN의 레이어 구성
- [ ] 차원수와 모델 용량
- [x] 회전/대칭 상태를 동일하게 처리하는 테크닉 → PR#6에서 D4 대칭 증강으로 구현
- [ ] 학습 과정 시각화
- [ ] 2048 구현 PR 검토하기
- [ ] 게임 시각화
- [x] 게임 플레이 UI 생성
...
- [ ] Q값은 증가하기만 하는지 확인
- [x] TD 발산하는 원인 분석 → PR#6에서 Huber Loss, 노름 클리핑, 보상 스케일링, bootstrap explosion 방지로 해결
- [x] MC가 랜덤과 다를바 없는 이유 분석 → PR#6에서 MC에 gamma 할인 적용, D4 증강, 보상 스케일링 추가
- [x] 맨처음 input 데이터로 512 같은 값 넣는지 확인 → PR#6에서 log2 정규화 → 원핫 인코딩(16채널)으로 변경, 값 크기가 직접 입력되지 않음
- [x] 애초에 수가 너무 클때 ReLU, loss^2 이런것들이 망하는 이유 분석 → PR#6에서 MSE→Huber Loss, 보상 스케일링(score/100), 원핫 인코딩으로 해결
