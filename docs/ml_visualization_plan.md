...
| 항목 | 값 |
|------|-----|
| 구조 | 32 → 256 → 256 → 4 (3층 FC) |
| 입력 인코딩 | 투채널 (셀당 2값: 존재 여부 + log₂(v)/16 = 4×4×2 = 32) |
| 총 파라미터 수 | 75,268개 |
| 메모리 (float64) | 약 588 KB |
...
입력은 투채널 인코딩(32차원, 셀당 존재 여부 + log₂(v)/16), 손실은 Huber Loss, 클리핑은 노름 기반입니다.
