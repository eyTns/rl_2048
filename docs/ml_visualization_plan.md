...
근거:
- 모델이 19,204 파라미터로 작아 JS에서 충분히 연산 가능
...
128개 뉴런을 전부 개별 표시하면 연결선이 18,944개가 되어 표시 불가능합니다.

**해결: 뉴런 그룹화**

| 층 | 실제 뉴런 수 | 표시 노드 수 | 방법 |
|----|-------------|-------------|------|
| 입력 | 16 | 16 | 전부 표시 (4×4 보드 셀) |
| 은닉1 | 128 | 16 | 8개씩 묶어 16그룹 |
| 은닉2 | 128 | 16 | 8개씩 묶어 16그룹 |
| 출력 | 4 | 4 | 전부 표시 (↑↓←→) |
...
예상 JSON 크기: ~350KB (float 텍스트 표현이 바이너리보다 큼)
...
```
QNetwork {
    constructor(hiddenSize=128)   // Xavier 초기화
...
`trainer.py`의 TD/MC 트레이너를 JavaScript로 포팅합니다.
