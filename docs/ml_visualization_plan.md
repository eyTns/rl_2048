...
| 항목 | 값 |
|------|-----|
| 구조 | 16 → 256 → 256 → 4 (3층 FC) |
| 입력 인코딩 | 정규화 지수 (log₂(v)/16, 셀당 1값 = 4×4 = 16) |
| 총 파라미터 수 | 132,612개 |
...
입력은 정규화 지수 인코딩(16차원, log₂(v)/16), 손실은 Huber Loss, 클리핑은 노름 기반입니다.
...
순수 JS 환경에서 학습 연산 속도를 개선하기 위해 적용한 최적화 목록.

### 검토 결과 요약

| 항목 | 설명 | 판정 |
|------|------|------|
| A. 리워드 스케일링 최적화 | scaleReward를 룩업 테이블로 교체 | 불채택 — 연산 비중 미미, 효과 없음 |
| B. augmentBoard 캐싱 | 동일 보드의 D4 증강 결과 캐싱 | 불채택 — 복잡도 대비 효과 적음 |
| **C. 중복 forward() 제거** | getAction + 별도 forward 호출 통합 | **적용 완료** |
| **D. 연산 버퍼 재사용** | forward/backward 임시 배열 할당 제거 | **적용 완료** |
| **E. renderBoard DOM 재사용** | 매 프레임 16개 div 파괴/재생성 제거 | **적용 완료** |

### C. 중복 forward() 제거

`getAction`이 `{ action, qValues }`를 반환하도록 변경하여, 호출부에서 Q값 표시용으로 forward()를 다시 호출하던 패턴을 제거.

변경 파일: `qnetwork.js`, `trainer.js`, `ui.js`

- TDTrainer: getAction 직후 UI용 forward 1회 제거
- MCTrainer: getAction 직후 에피소드 수집용 forward 1회 제거
- ui.js btnPlay/btnStep: getAction 직후 Q값 표시용 forward 1회 제거

### D. 연산 버퍼 재사용 (0 GC 할당)

변경 파일: `qnetwork.js`

**핵심: 매 forward/backward 호출마다 수십 개의 임시 배열을 생성하던 것을 생성자에서 1회 할당한 버퍼를 재사용하는 방식으로 전환.**

1. `_allocBuffers()`: 생성자에서 forward 활성화 버퍼(x, z1, a1, z2, a2, z3), backward 기울기 버퍼(dz3, da2, dz2, da1, dz1), 가중치 기울기 버퍼(dw1~3, db1~3) 사전 할당
2. 모든 연산 함수를 in-place 버전으로 교체:
   - `matMul` → `matMulInto` (출력 버퍼 인자, i,p,j 루프 순서로 캐시 친화적)
   - `addBias` → `addBiasInplace` (새 배열 생성 없이 덧셈)
   - `.map(r => r.map(v => Math.max(0,v)))` → `reluInto` / `reluBackwardInto`
   - `matMul(transpose(a2), dz3)` → `outerProductInto(a2[0], dz3[0], dw3)` — 전치 배열 생성 자체를 제거
   - `matMul(dz3, transpose(w3))` → `vecMatTransposeInto(dz3[0], w3, da2[0])` — 전치 없이 W[j][p] 접근
3. `_clipNorm` → `_clipNormInplace`: norm 초과 시 원본 배열에 직접 scale 적용

### E. renderBoard DOM 재사용

변경 파일: `ui.js`

- `initGrid()`: 페이지 로드 시 및 새 모델 생성 시 16개 div를 1회 생성
- `renderBoard()`: 기존 div의 `className`/`textContent`만 변경 (innerHTML 파괴+재생성 제거)

### 추가 가속 후보 (미적용)

| 방법 | 예상 가속 | 난이도 |
|------|----------|--------|
| Flat 1D 배열 + 수동 인덱싱 | 1.5~2배 | 낮 |
| WebAssembly (matMul 핵심 루프) | 5~10배 | 중 |
| Web Worker 병렬화 (D4 증강) | 2~4배 | 중 |
| WebGPU compute shader | 10~50배 | 높 |
